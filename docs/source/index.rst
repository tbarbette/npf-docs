...
.. tabs::
   ...
   .. tab:: History
      ...
      NPF includes `npf-regress` and `npf-watch` to track performance over time across commits. See the :ref:`Regression Testing <regress>` page.

   .. tab:: Comparison

      You can easily compare multiple versions or branches of a repository using ``--compare-version`` or ``--branch`` flags.
...
By passing the ``--web <filename>`` option, a fully interactive, standalone HTML page is generated. See the :ref:`Output options <output>` for more details.
...
.. tabs::
   ...
   .. tab:: Localhost
      ...
      Use ``--cluster client=localhost`` to run experiments locally. See the :ref:`Cluster documentation <cluster>` for more setup options.

   .. tab:: Local Cluster

      Define ``.node`` files and map roles with ``--cluster client=my_server`` to connect via SSH.

   .. tab:: Grid5000

      NPF integrates with Enoslib to dynamically reserve and provision resources on Grid5000. See the :ref:`Enoslib Integration <enoslib_integration>` page.
